...
In this final assignment a non-thresholded gene set enrichment analysis will be conducted followed by a visualization of the results in Cytoscape. with the use of the EnrichmentMap pipeline.  Finally, a post analysis will be conducted examining potential drugs being used in the treatment of these diseases.
...
```{r, eval=FALSE}
# make a file just the genes and the ranks
rankGenesOnly <- "rank_list2.tsv"
readr::write_tsv(rankedGenes, rankGenesOnly)
```

```{r}
outputFile <- "output_hits.rds"
if (file.exists(outputFile)) {
  a <- readRDS(outputFile)
}

```
# Non-thresholded Gene set Enrichment Analysis

Tried using the GUI and now trying with `fgsea`.
```{r}
# download the with IEA genesets
```

```{r}

# Change to use the with IEA and see difference
...
# view the results in decreasing NES, the bottom of the list
...
Save gostupfilteredresults and gostdownfilteredresults to compare and the results for each annotation source.
...
AutoAnnotate was used to annotate the network.  The parameters used were the MCL Cluster algorithm and annotate with a maximum of 50 annotations to allow for a more readable network.  The label column was GS_DESCR.  The option to layout clusters to minimize overlap was chosen to further increase the readability of the graph.


**3. Make a publication ready figure - include this figure with proper legends in your notebook.**

Below in Figure 2 is the publication ready figure.  Figure 3 is a closeup of the main cluster.

![Figure 2: Enrichment map annotated with AutoAnnotate](figures/overall_network_annotated.png)

![Figure 3: Close up of the main cluster in annotated enrichment map](figures/closeup_main_clusters.png)

![Figure 4: Close up of smaller cluster in annotated enrichment map](figures/closeup_smaller_cluster.png)

![Figure 5:  Pub ready figure](figures/publication_ready_figure.png)


**4. Collapse your network to a theme network. What are the major themes present in this analysis? Do they fit with the model? Are there any novel pathways or themes?**

![Figure 6: Collapsed theme network](figures/collapsed_theme_network.png)

The major themes can be seen in Figure 7.  They include pathways involving the human epidermal growth factor and anti arrhythmic drugs.  Considering this study looked at samples with heart disease (DCM, ICM, and NF) it is not surprising to find themes regarding arrhythmias of the heart.  Having these heart diseases can cause irregular heart rhythms.  Regarding epidermal growth factors, this is also consistent with the samples/disease being studied.  DCM and ICM are subtypes of cardiomyopathy which is a heart disease making it harder for the heart to pump blood to the rest of the body so with this knowledge its possible pathways involving muscles are enriched.
